...
These are my notes on the Go programming language.

## Basics

- Each Go file declares a package.
- Style is enforced by the `go fmt` tool. Go expects
  certain styling to compile.
...
- Go constructs strings from **runes**, which are unicode characters. Unicode
  characters are also useable in the language syntax itself.

Documentation is at [golang.org](golang.org). It's also available using the `go
doc` tool.
...
Go dependencies are done with Go Modules.
...
The assignment operator `:=` will also allow you to redeclare some variables,
as long as part of the declaration statement includes new variables.
...
Outside functions, `const` blocks declare global constants and `var` blocks
declare global variables. Capitalized constants and variables export them for
use by outside modules.
...
will interpret special characters, like `\n`.
...
Slices can be formed on the fly from arrays or slices. The first value is the
starting index (inclusive), the second value is the ending index (exclusive).
...
Arrays are fixed in memory and are only passed between functions by copying,
whereas slices are passed by reference. Slices are more performant when passed
between functions because it doesn't copy data, and allows array data to be
directly modified outside of its declaration scope.

Dynamic slices can be created using the `make` function, which allocates
memory.
...
Attempting to access sections of a slice or array that do not exist will cause
...
Maps can be instantiated using the `map` keyword, either by `make` or by
explicit definition.
...
Accessing an element of the map that doesn't exist will return the zero type.
An error can be explicitly thrown by checking for a return value.
...
Iterating over a map uses `range`. Maps are not sorted by default, so results
will come out in arbitrary order.
...
Items can be deleted from maps. Deletion will silently pass if the key does not
exist.
...
Go can use if statements for error handling. Variables declared within if
statements are only accessible within the conditional block.
...
Switch statements do not have a necessary `break` and do not fall-through.
Instead, the `fallthrough` keyword will cause cases to collapse.
...
Go does have post-fix updating, `foo++`, but it isn't a statement and is only
valid inside loop clauses.
...
The `defer` keyword queues up actions to be taken after a function exits.
Statements which are defered execute in stack order. Useful for function
clean-up.
...
Return values in Go can also be defined at execution time for shorthand
returns.
...
In general, pass-by-reference is accomplished by using pointers. Otherwise, Go
usually uses pass-by-copy.

## Errors

Go has errors which return from functions and (very rarely used) panics to
...
Defining errors can be done using error formatting, `Errorf` or the `errors`
library:
...
- `errors` can be used to define custom error types. Downstream behavior can
...
`panic` and `recover` in Go can stop execution or return stack traces if
something happens that is not recoverable.
